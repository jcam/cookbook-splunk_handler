    ...
    class SplunkHandler < Chef::Handler
      ...
      def initialize(username, password, host, port='8089', index='main',
        ...
        @splunk_url = URI::HTTP.new(
          scheme, "#{username}:#{password}", host, port, nil, nil, nil, nil, nil)
        @index = index
      end
      ...
      def splunk_post(event, metadata)
        ...
        client = Chef::REST.new(@splunk_url)
        client.raw_http_request(:POST, client.create_url(endpoint_path), {}, event)
      end
    end
  end
end
